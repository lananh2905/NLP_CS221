...
def pad_sequence_sentence(sentence):
    ...
    # Nếu câu không có từ nào → trả mảng toàn 0 đúng shape
    if array is None or len(array) == 0:
        array = np.zeros((1, 200))

    # Nếu chỉ là vector 1 chiều → ép thành 2D
    if array.ndim == 1:
    ...
    DESIRED_SEQUENCE_LENGTH = 205
    length_diff = DESIRED_SEQUENCE_LENGTH - seq_len

    if length_diff > 0:
        pad = np.zeros((length_diff, embedding_dim))
        array = np.concatenate([array, pad], axis=0)
    elif length_diff < 0:
        array = array[:DESIRED_SEQUENCE_LENGTH, :]

    array = np.expand_dims(array, axis=0)  # shape: (1, 205, 200)
    return array
